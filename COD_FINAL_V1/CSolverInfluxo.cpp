...
void CSolverInfluxo::Simular()
{
    ...
    while(!teste)
    {
						...
						case '1':
							{
						        ...
						        plot.savetops("dois-pocos-stiles");
        ...
        {	
        	...
        	vector<double> vazao;
	        vazao = dykstra.VazaoInjecao(reservatorio.GetKrw(), camadas.GetMw(), camadas.GetLargura(), camadas.GetDp(), camadas.GetK(), camadas.GetBw(), camadas.GetComprimento(), camadas.GetEspec(), camadas.GetM());
			...
			vector<double> posicao;
	        posicao = dykstra.PosicaoAguaInjetada(camadas.GetK(), camadas.GetLargura(), camadas.GetM());  
		...
		do
		{
			...
			if (f == '1'){
				...
				plot4.savetops("Fluxo-fracionario-de-agua");
